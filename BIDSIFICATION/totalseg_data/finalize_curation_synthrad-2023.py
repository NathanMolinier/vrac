...
def create_json_file(path_file_out):
    ...
    data_json = {
        ...
        "GeneratedBy": [
            {
                "Name": "TotalSegmentatorCT",
                "Version": "v2.0.4",
                "Description": "Automatically generated by Yehuda Warszawer (https://github.com/wasserth/TotalSegmentator)",
...
def create_dataset_description(path_output, dataset_name, derivative=False):
    ...
    if not derivative:
        data_json = {
            ...
            "DatasetType": "raw",
            "Authors": [
                    "https://aapm.onlinelibrary.wiley.com/doi/full/10.1002/mp.16529",
                    "Yehuda Warszawer",
                    "Nathan Molinier"
        ]
        }
...
def main():
    ...
    # Spine values
    spine_val = [i for i in range(18,42)]+[92]
            ...
            for file_name in os.listdir(path_sub_der_dir):
                if '.nii.gz' in file_name:
                    ...
                    if '_seg' in file_name:
                        ...
                        label_spine = zeros_like(label)
                        for val in spine_val:
                            label_spine.data[np.where(label.data==val)] = val
                            label.data[np.where(label.data==val)] = 0

                        # Check image size
                        img_path = os.path.join(path_dataset, sub, 'anat', file_name.replace('_seg',''))
                        img = Image(img_path)
                        if img.data.shape != label.data.shape:
                            raise ValueError('Image and label have different size')
                    
                        # Save files
                        spine_path = file_path.replace('_seg', '_label-spine_dseg')
                        multi_path = file_path.replace('_seg', '_label-mutli_dseg')
                        label_spine.save(spine_path)
                        label.save(multi_path)
                        create_json_file(spine_path)
                        create_json_file(multi_path)
                    else:
