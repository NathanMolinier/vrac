...
def create_json_sidecar(path_output, labels):
    ...
    data_json = {
        ...
        "GeneratedBy": [
            {
                "Name": "Semi-automatic",
                "Author": "https://pubs.rsna.org/doi/10.1148/radiol.241613",
            }
        ],
        "Labels": labels
    }
...
def normalize_filename(subject_bids, label=False):
    if label:
        return f"{subject_bids}_MR_label-body_dseg.nii.gz"
    else:
        return f"{subject_bids}_MR.nii.gz"
...
def main(path_dataset, path_output):
    ...
    for di in img_dirs:
        path_file_in = os.path.join(path_dataset, di, "mri.nii.gz")
        ...
        if len(seg_files) != 50:
            raise ValueError(f"Expected 50 segmentation files in {segmentations_folder}, found {len(seg_files)}")
        if labels is None:
    ...
    logger.info(f'Successfully processed {processed_images} MRI images')
